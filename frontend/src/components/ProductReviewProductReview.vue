<template>
    <v-card style="width:450px; height:100%;" outlined>
        ...
        <v-card-title v-if="value._links">
            제품 후기 # {{decode(value._links.self.href.split("/")[value._links.self.href.split("/").length - 1])}}
        </v-card-title >
        <v-card-title v-else>
            제품 후기
        </v-card-title >        
        ...
        <v-card-text>
            <String v-if="editMode" label="ReviewId" v-model="value.reviewId" :editMode="editMode" :inputUI="''"/>
        ...
        <v-card-actions>
            ...
            <v-dialog v-model="leaveReviewDiagram" width="500">
                <LeaveReviewCommand
                    @closeDialog="closeLeaveReview"
        ...
        methods: {
            ...
            async save(){
                try {
                    ...
                    if(!this.offline) {
                        if(this.isNew) {
                            temp = await axios.post(axios.fixUrl('/productReviews'), this.value)
                        } else {
